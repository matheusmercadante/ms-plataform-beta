...
const GET_CATEGORIES = gql`
  query getCategories($query: String, $orderBy: CategoryOrder) {
    getCategories(query: $query, orderBy: $orderBy) {
      totalCount
      edges {
        cursor
        node {
...
export default function Category() {
  ...
  function onAllCheck(event) {
    if (event.target.checked) {
      const idx = data && data.getCategories.edges.map((category) => category.node.id);
          ...
          <Wrapper style={{ boxShadow: "0 0 5px rgba(0, 0 , 0, 0.05)" }}>
            <TableWrapper>
              <StyledTable $gridTemplateColumns="minmax(70px, 70px) minmax(70px, 70px) minmax(70px, 70px) minmax(150px, auto) minmax(150px, auto) auto">
                ...
                {data ? (
                  data.getCategories.totalCount !== 0 ? (
                    data.getCategories.edges
                      ...
                      .map((row, index) => (
                        <React.Fragment key={index}>
                          ...
                          <StyledCell>{row[1].name}</StyledCell>
